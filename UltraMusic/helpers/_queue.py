...
class Queue:
    ...
    def remove_chat(self, chat_id: int) -> None:
        """Remove a chat's queue entry entirely if the queue is already empty.

        Calling ``clear()`` empties the deque but leaves the key in the
        ``defaultdict``.  Over time this accumulates one dead entry per chat
        the bot has ever served.  Call this method right after ``clear()``
        (or whenever a chat stops) to drop the key and free the memory.
        The guard ``not self.queues[chat_id]`` makes it safe to call even if
        the queue was never populated – in that case the defaultdict *just*
        created an empty deque; we delete it immediately.
        """
        if chat_id in self.queues and not self.queues[chat_id]:
