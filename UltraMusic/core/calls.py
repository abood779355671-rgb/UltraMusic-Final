...
from UltraMusic.helpers._themes import THEMES, get_theme, build_now_playing_text
...
class TgCall(PyTgCalls):
    def __init__(self):
        ...
        self._auto_leave_tasks: dict[int, asyncio.Task] = {}  # auto-leave timers keyed by chat_id
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # AUTO-LEAVE EMPTY CALL LOGIC
    # ══════════════════════════════════════════════════════════════════════════

    def _cancel_auto_leave(self, chat_id: int) -> None:
        """Cancel any pending auto-leave timer for chat_id and clean up."""
        task = self._auto_leave_tasks.pop(chat_id, None)
        if task and not task.done():
            task.cancel()

    def cancel_all_auto_leave_tasks(self) -> None:
        """Cancel every active auto-leave task (called when feature is globally disabled)."""
        for chat_id in list(self._auto_leave_tasks):
            self._cancel_auto_leave(chat_id)

    async def _auto_leave_watcher(self, chat_id: int) -> None:
        """
        Background task: watch one call for empty state and leave after timeout.

        Flow:
        1. Poll every 60 s to check whether any *human* participants remain.
        2. On first empty poll: start countdown using the configured minutes.
        3. If the call fills again before timeout: reset countdown.
        4. If still empty after timeout: call self.stop(chat_id) and notify.
        5. If the global switch is turned off mid-run: exit silently.
        6. Skips channel-play chats (cmode != None).
        """
        POLL_INTERVAL = 60  # seconds between participant checks

        # ── مدة الانتظار (قابلة للتغيير من DB) ──────────────────────────────
        try:
            minutes = int(await db.get_setting("auto_leave_empty_minutes", default=5))
        except Exception:
            minutes = 5
        leave_timeout = minutes * 60  # convert to seconds

        empty_since: float | None = None  # monotonic timestamp of first empty detection
        ...
        try:
            while True:
                ...
                # ── 1. سويتش عالمي ─────────────────────────────────────────
                try:
                    enabled = await db.get_setting("auto_leave_empty_enabled", default=True)
                except Exception:
                    enabled = True
                if not enabled:
                    # Feature was disabled globally while we were running → exit
                    empty_since = None
                    return

                # ── 2. تأكد أن المكالمة لا تزال نشطة ──────────────────────
                if not await db.get_call(chat_id):
                    return  # Call ended by other means

                # ── 3. تجاهل وضع channel play ───────────────────────────────
                try:
                    cmode = await db.get_cmode(chat_id)
                except Exception:
                    cmode = None
                if cmode is not None:
                    empty_since = None
                    return  # channel play mode — do not auto-leave

                # ── 4. اقرأ المشاركين عبر pytgcalls ─────────────────────────
                try:
                    client = await db.get_assistant(chat_id)
                    participants = await client.get_participants(chat_id)
                except Exception:
                    # Can't read participants (call gone / not connected) — abort
                    empty_since = None
                    return

                # استثن المساعد نفسه — "خالية" = صفر مستخدمين بشريين
                # get_participants() يُرجع قائمة GroupCallParticipant؛
                # كل مشارك لديه is_self (True للمساعد نفسه).
                human_count = sum(
                ...
                if human_count > 0:
                    # There are real listeners — reset countdown
                    empty_since = None
                else:
                    # No humans in call
                    now = asyncio.get_running_loop().time()
                    if empty_since is None:
                        empty_since = now  # first empty detection
                    elif now - empty_since >= leave_timeout:
                        # ── 5. تنفيذ المغادرة ──────────────────────────────
                        # Send notification first (best-effort)
                        try:
                            _lang = await lang.get_lang(chat_id)
                            msg_text = _lang.get(
                                "auto_leave_empty",
                                "🚪 غادرتُ المكالمة الصوتية تلقائياً لعدم وجود مستمعين."
                            )
                            await app.send_message(chat_id=chat_id, text=msg_text)
                        except Exception as _notify_err:
                            logger.debug(
                                f"[auto_leave] failed to send notification for {chat_id}: {_notify_err}"
                            )

                        logger.info(
                            f"[auto_leave] leaving {chat_id} — empty for "
                            f"{int(now - empty_since)}s (threshold={leave_timeout}s)"
                        )
                        # self.stop() also calls _cancel_auto_leave → safe re-entry
                        await self.stop(chat_id)
                        return  # task ends naturally after stop

        except asyncio.CancelledError:
            # Graceful cancellation (user joined / feature disabled / stop() called)
            return
        finally:
            # Always clean up our own entry to avoid stale references
            self._auto_leave_tasks.pop(chat_id, None)

    async def _schedule_auto_leave(self, chat_id: int) -> None:
        """
        Ensure exactly one auto-leave watcher task is running for chat_id.
        Called whenever a call becomes active (add_call path → play_media).
        Skips channel-play chats immediately.
        """
        # Global switch check (fast path, no DB if already disabled)
        try:
            enabled = await db.get_setting("auto_leave_empty_enabled", default=True)
        except Exception:
            enabled = True
        ...
        # Cancel any stale task before creating a fresh one
        self._cancel_auto_leave(chat_id)

        task = asyncio.create_task(
            self._auto_leave_watcher(chat_id),
            name=f"auto_leave_{chat_id}",
        )
        self._auto_leave_tasks[chat_id] = task

    async def stop(self, chat_id: int) -> None:
        # ── Cancel any pending auto-leave watcher for this chat ──────────────
        self._cancel_auto_leave(chat_id)
        ...
        # ── Fix: release _play_next_locks entry ──────────────────────────────
        # Only remove the lock when it is not actively held; a concurrent
        # play_next() call may still be inside `async with lock:` at this
        # instant (e.g. when play_media raises and calls stop() internally).
        _lock = self._play_next_locks.get(chat_id)
        if _lock is not None and not _lock.locked():
            self._play_next_locks.pop(chat_id, None)

        # ── Fix: drop empty queue dict entry ─────────────────────────────────
        # defaultdict leaves a key with an empty deque after clear(); remove it
        # now so self.queues does not accumulate one entry per chat forever.
        queue.remove_chat(chat_id)
    ...
    async def play_media(
        ...
        if config.THUMB_GEN and media.thumbnail:
        ...
        try:
            ...
            if not seek_time:
                ...
                # ── جدوِل مراقب auto-leave بعد تسجيل المكالمة مباشرةً ──────
                try:
                    await self._schedule_auto_leave(chat_id)
                except Exception as _al_err:
                    logger.debug(f"[auto_leave] schedule error for {chat_id}: {_al_err}")

                # ── تحميل الثيم النشط (active_theme) من DB ────────────────
                # أي فشل هنا (DB غير متاحة، ثيم غير موجود...) يُرجع classic
                # فوراً، ولا يجب أن يخرج أي استثناء من هذه الخطوة على الإطلاق.
                try:
                    theme_id = await db.get_active_theme() or "classic"
                    theme = await get_theme(theme_id)
                except Exception:
                    logger.warning("فشل تحميل الثيم، استخدام classic")
                    theme = THEMES["classic"]

                # ── قراءة قالب رسالة التشغيل: DB أولاً، ثم الثيم، ثم ar.json ──
                try:
                    _play_template = await db.get_setting(
                        "play_media_template", None
                    )
                    if not _play_template:
                        _play_template = theme.get(
                            "message_template") or _lang["play_media"]
                except Exception as _tmpl_err:
                    logger.warning(
                        f"[play_media] تعذّر قراءة play_media_template من DB، "
                        f"الرجوع للافتراضي: {_tmpl_err}"
                    )
                    _play_template = _lang["play_media"]
                text = build_now_playing_text(
                    {"message_template": _play_template},
                    media.url,
                    media.title,
                    media.duration,
                    media.user,
                ...
                if not media.is_live and media.duration_sec:
                    ...
                    keyboard = await buttons.controls(chat_id, timer=timer_text)
                else:
                ...
                # ── إرسال الرسالة وفق show_thumbnail في الثيم ─────────────
                # أي خطأ هنا يُعيد سلوك play_media القديم تماماً (إرسال صورة).
                try:
                ...
                if not show_thumbnail:
                    try:
                        sent_photo = await app.send_message(
                        ...
                        logger.warning(
                            f"[play_media] فشل الإرسال بدون صورة (show_thumbnail=False)، "
                            f"الرجوع لسلوك الصورة الافتراضي: {_send_err}"
                        )
        ...
        async def update_handler(_, update: types.Update) -> None:
            if isinstance(update, types.StreamEnded):
                ...
                if update.status in [
                    ...
                    hasattr(types.ChatUpdate.Status, "PARTICIPANT_JOINED")
                    and update.status == types.ChatUpdate.Status.PARTICIPANT_JOINED
                ):
                    # A human joined the call → cancel any pending auto-leave countdown
                    # (PARTICIPANT_JOINED may not exist in all py-tgcalls 2.x versions)
                    self._cancel_auto_leave(update.chat_id)
