...
@app.on_message(command(["تقديم", "ترجيع"]) & filters.group & ~app.bl_users)
@lang.language()
@can_manage_vc
async def _seek(_, m: types.Message):
    ...
    current_time = max(1, getattr(media, 'time', 1))
    if m.command[0] == "ترجيع":
    ...
    if success:
        ...
        await sent.edit_text("❌ فشل التقديم!")
