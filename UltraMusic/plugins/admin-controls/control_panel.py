...
async def _listen(
    ...
    """
    بديل لـ app.listen() من pyromod — يعمل مع kurigram و pyrogram العادي.
    يُسجَّل handler مؤقت ينتظر رسالة تطابق الـ filters ثم يُزال تلقائياً.
    يرفع asyncio.TimeoutError عند انتهاء المهلة.
    """
    loop = asyncio.get_event_loop()
    future: asyncio.Future = loop.create_future()

    async def _tmp_handler(client: pyrogram.Client, message: types.Message):
        if not future.done():
            future.set_result(message)

    handler_ref = app.add_handler(
        pyrogram.handlers.MessageHandler(_tmp_handler, filters),
        group=999,
    )
