...
from UltraMusic import app, db, lang, stop


# ------------------------------------------------------------------------------
# دالة مشتركة للإعادة تشغيل — يستخدمها الأمر /restart ولوحة التحكم (panel:system)
# ------------------------------------------------------------------------------

async def _do_restart() -> None:
    """
    تنفّذ منطق إعادة التشغيل: تنظيف cache ثم os.execl.
    مُشتركة بين /restart ولوحة التحكم — لا تُكرَّر الكود.
    """
    for directory in ["cache"]:
        shutil.rmtree(directory, ignore_errors=True)
    asyncio.create_task(stop())
    await asyncio.sleep(2)
    os.execl(sys.executable, sys.executable, "-m", "UltraMusic")
